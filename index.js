...
function listAllCourses(students) {
  ...
  students.forEach((student) => {
    student.courses.forEach((course) => {
      if (!uniquecourses.includes(course)) uniquecourses.push(course);
    });
  });
...
function removeCourseFromStudent(student, course) {
  // start here
  const res = student.courses.filter((c) => {
...
// Omar's variant to shortenn the code
function removeCourseFromStudent2(student, course) {
  // start here
...
console.log(
  "OMARS CHEAT CODE!! 🤓\n",
  removeCourseFromStudent2(students[6], "Science")
);
